...
class TestSupabaseUser extends BaseAuthUser {
  TestSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> testSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TestSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
